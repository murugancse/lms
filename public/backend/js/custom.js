...
function imageName(e,id)
{
    let filename = e.target.files[0].name;
    alert(filename);
    $('#placeholderFileOneName_'+id).val(filename);
}
/*var fileInput = document.getElementById(concat('document_',parent_id));
if (fileInput) {
    fileInput.addEventListener('change', showFileName);
    function showFileName(event) {
        var fileInput = event.srcElement;
        var fileName = fileInput.files[0].name;
        document.getElementById(concat('placeholderFileOneName_',parent_id)).placeholder = fileName;
    }
}*/
...
if (fileInput) {
    ...
    function showFileName(event) {
        var fileInput = event.srcElement;
        var fileName = fileInput.files[0].name;
        document.getElementById('placeholderFileThreeName3').placeholder = fileName;
    }
}
...
$(document).ready(function() {
    ...
    $(".promote_session").change(function() {
        var url = $('#url').val();
                ...
                $.each(data, function(i, item) {
                    if (item.length) {
                        ...
                        $.each(item, function(i, session_class) {
                            ...
                            $("#select_class_div ul").append("<li data-value='" + session_class.id + "' class='option'>" + session_class.class_name + "</li>");
                        });
                    } else {
    ...
    $("#btn-assign-fees-group").click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                console.log(data);
                setTimeout(function() {
                    toastr.success('Successfully assigned Fees Group!', 'Success Alert', {
    ...
    $("#btn-assign-fees-discount").click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                console.log(data);
                setTimeout(function() {
                    toastr.success('Successfully assigned Fees Discount!', 'Success Alert', {
                        "iconClass": 'customer-info'
                    }, {
                        timeOut: 2000
                    });
...
$(document).on("change", "#question-type", function(event) {
    ...
    if (question_type == "") {
        ...
        $('#question_bank div#true-false').hide();
        $('#question_bank div#fill-in-the-blanks').hide();
        $('#question_bank div#image-question').hide();
        $('#question_bank div#multiple-options').html('');
    } else if (question_type == "M" || question_type == "MM") {
        ...
        $('#question_bank div#multiple-options').html('');
    } else if (question_type == "IA") {
       
        $('#question_bank div#multiple-choice').hide();
        $('#question_bank div#true-false').hide();
        $('#question_bank div#image-question').show();
       // $('#question_bank div#fill-in-the-blanks').show();
        $('#question_bank div#multiple-options').html('');
    } else {
        $('#question_bank div#multiple-choice').hide();
...
$("form#assign_subject").on("submit", function() {
    ...
    if (abc != "") {
        setTimeout(function() {
            toastr.error('May duplicate entry uccured, please check and try again!', 'Error Alert', {
                timeOut: 5000
            });
        }, 500);
...
$(document).ready(function() {
    ...
    $("#select_academic_year").change(function() {
        var url = $('#url').val();
        var formData = {
            year: $(this).val()
        };
                ...
                $.each(data, function(i, item) {
                    if (item.length) {
                        ...
                        $.each(item, function(i, class_info) {
                            ...
                            $("#select_class_div ul").append("<li data-value='" + class_info.id + "' class='option'>" + class_info.class_name + "</li>");
                        });
                    } else {
                        $('#select_class_div .current').html('SELECT CLASS *');
                        $('#select_class').find('option').not(':first').remove();
